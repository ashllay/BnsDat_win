...
{
	printf(("Operation Mode: Dump File\n\n") + file + ("\n\n"));
...
{
	printf(("Operation Mode: Transform File\n\n") + file + ("\n\n"));
...
{
	printf(("Operation Mode: Translate Files\n\n") + directory + ("\n\n"));
    wxString file;
    ...
    {
		printf("Missing Dictionary: \'translation.xml\'\n");
        return;
    }
    BI18N translator;
...
void extract(wxString file, bool convert)
{
    if(convert)
    {
		printf(("Operation Mode: Extract Files (Plain)\n\n") + file + ("\n\n"));
    }
    else
    {
		printf(("Operation Mode: Extract Files (RAW)\n\n") + file + ("\n\n"));
    }
    wxFFileInputStream fis(file);
...
bool bnsApp::OnInit()
{
    ...
    if(instruction != wxT("-e") &&
       ...
       instruction != wxT("-c"))
    {
		printf("First argument is not a valid instruction\n");
        exit(2);
    }
    ...
    if(instruction == wxT("-e"))
    {
        ...
        {
			printf("Second argument is not a valid file\n");
            exit(3);
        }
        else
        {
